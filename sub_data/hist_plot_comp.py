...
def draw_hist_v1(image_dir):
    ...
    for _, (image_tensors,targets) in enumerate(tqdm(train_loader)):        
        ...
        comp_cls_labels = classify_compressibility_scores_4class(targets)
        class_labels.append(comp_cls_labels.cpu().detach().numpy())
    
    y = np.concatenate(y, axis=0)
    class_labels = np.concatenate(class_labels, axis=0)
    
    print(y.shape, y.min(), y.max())
    print(class_labels.shape, class_labels.min(), class_labels.max())
    
    print(y.shape)
    np.save('./data/y_real-compressibility.npy', y)
    np.save("./data/y_real_4class_labels.npy", class_labels)
