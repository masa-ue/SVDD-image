...
from model import ResidualBlock, ThreeLayerConvNet
...
wandb.init(project="RCGDM-compressibility", config=config)
logger.info(f"\n{config}")
...
convnet = ThreeLayerConvNet(latent_dim, num_classes=5).to(device)
...
encoded_images = torch.load("./data/encoded_images_91361.pth")
...
comp_labels = np.load('./data/y_real_4class_labels.npy')
my_targets = torch.tensor(comp_labels, dtype=torch.int64)
...
for epoch in tqdm(range(num_epochs), desc="Epoch"):
    ...
    for i, (inputs, targets) in enumerate(tqdm(train_loader, desc="Training Progress")):
        ...
        # if i % 1000 == 0 and i != 0:
        #     torch.save(convnet, f'comp_model/4cls_comp_convnet_epoch_{epoch}_iter_{i}.pth')

    wandb.log({"epoch_loss": epoch_loss/(i+1)})
    logger.info(f"epoch_loss: {epoch_loss/(i+1):.4f}")

    torch.save(convnet, f'./comp_model/4cls_comp_convnet_{epoch}.pth')