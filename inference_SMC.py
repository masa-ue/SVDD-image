...
run_name = run_name + '_' + unique_id + 'SMC' + 'horse'
...
for i in tqdm(range(args.num_images // args.bs), desc="Generating Images"):
    ...
    if i == 0:
        eval_prompts = ['dog' for _ in range(args.bs) ]
    elif i == 1:
        eval_prompts = ['cat' for _ in range(args.bs) ]
    elif i == 2:
        eval_prompts = ['dog' for _ in range(args.bs) ] 
    elif i == 3:
        eval_prompts = ['monkey' for _ in range(args.bs) ] 
    elif i == 4:
        eval_prompts = ['rabbit' for _ in range(args.bs) ]  
    elif i ==5: 
        eval_prompts = ['butterfly' for _ in range(args.bs) ] 
    else:
        eval_prompts = ['panda' for _ in range(args.bs) ] 
